...
Чтобы вставить данные в текст сообщения, используйте синтаксис `{{ .ВидДанных.НазваниеПоля }}`. Просто вставьте его в нужное место вашего текста.
...
В `save_to_var` нужно использовать модификаторы, но нельзя использовать их одновременно:
- `goto`: этот модификатор позволяет перейти к другому меню после ввода данных.
- `do_button`: этот модификатор позволяет выполнить действие (например, [выполнить команду на стороне сервера](#как-выполнить-команду-на-стороне-сервера)) после получения сообщения от пользователя.
