...
В команду можно передать данный относящиеся к структуре объекта User (Пользователь) в формате `{{ .User.НазваниеПоля }}`

В команду также можно передать данный передаваемые пользователем через `{{ .Var.НазваниеПеременной }}`, но перед этим пользователь должен заполнить значения в переменные. Как заполнить переменные смотрите в [Как получить и сохранить текст введенный пользователем](#как-получить-и-сохранить-текст-введенный-пользователем)
  ...
  start:
    ...
    buttons:          
      - button:
          ...
          save_said: 
            save_to_var: city
            send_text: "Введите город в котором хотите узнать погоду"
            goto: weather_check
  weather_check:
    answer:
    ...
    buttons:
      ...
      - button:
          text: "Подтвердить"
          exec_button: "./scripts/example.sh {{ .User.UserId }} Город: {{ .Var.city }}"
```
